...
Purpose:           Validate Task 2.2 answers and the permitted change paths.
...
# Every new file below one of these prefixes is also permitted. Task 2.2 asks a
# student to add modules, so the boundary is a prefix set rather than a fixed
# file list.
ALLOWED_PREFIXES = ("src/api/extensions/", "tests/student/")
...
def main(root: Path | None = None, *, changed_paths: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("Task 2.2 answer verification passed.")
    return 0
